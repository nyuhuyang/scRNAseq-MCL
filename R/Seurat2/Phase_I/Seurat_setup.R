...
save(g2,file= paste0(path,"g2_43_20200130.Rda"))
...
GC()
pcs =1:75
system.time(object %<>% RunTSNE(reduction.use = "pca", dims.use = pcs, do.fast = TRUE))
system.time(object %<>% FindClusters(reduction.type = "pca", resolution = 0.6, dims.use = pcs,
                             save.SNN = TRUE, n.start = 10, nn.eps = 0.5,
                             force.recalc = TRUE, print.output = FALSE))
p0 <- DimPlot(object = object, reduction.use = "tsne", 
              pt.size = 0.3, group.by = "orig.ident", do.return = T,
              no.legend = T)
#======1.6 RunHarmony=======================
jpeg(paste0(path,"S1_RunHarmony_seurat2.jpeg"), 
...
system.time(object %<>% RunTSNE(reduction.use = "harmony", 
                                dims.use = pcs, do.fast = TRUE))
system.time(object %<>% FindClusters(reduction.type = "harmony", resolution = 0.6, dims.use = pcs,
                                 save.SNN = TRUE, n.start = 10, nn.eps = 0.5,
                                 force.recalc = TRUE, print.output = FALSE))


p3 <- TSNEPlot(object, do.return = T, pt.size = 0.3, 
               group.by = "orig.ident",no.legend = T)
p4 <- TSNEPlot(object, do.label = T, do.return = T, 
               pt.size = 0.3,no.legend = T)

jpeg(paste0(path,"S1_pca_vs_Harmony_TSNEPlot.jpeg"), 
     units="in", width=10, height=7,res=600)
