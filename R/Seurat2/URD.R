...
object %<>% SetAllIdent("X5_clusters")
colors <- ExtractMetaColor(object)
object <- RunUMAP(object = object, dims = 1:75,reduction.use = "harmony")
URD_object <- seuratToURD(object)
remove(object);GC()
pcSDPlot(URD_object)
jpeg(paste0(path,"UMAP_Stage_X5_clusters.jpeg"), units="in", width=10, height=7,
     res=600)
plotDim(URD_object,"X5_clusters",plot.title = "UMAP: Stage",label.clusters = T,
...
plotDimArray(URD_object, reduction.use = "dm", dims.to.plot = 1:8, 
             ...
             label="X5_clusters", plot.title="", legend=F)
