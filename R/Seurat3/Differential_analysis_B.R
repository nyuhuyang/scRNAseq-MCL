...
TSNEPlot.1(object,label = T,no.legend=T,cols = ExtractMetaColor(object),repel = T,
           do.print = F)
...
save(B_cells_MCL, file = "data/B_cells_MCL_43_20190615.Rda")

(load("data/B_cells_MCL_43_20190615.Rda"))
...
B_cells_MCL <- subset(B_cells_MCL, idents = 21, invert = TRUE)
B_cells_MCL <- sortIdent(B_cells_MCL,numeric = T)
g1 <- TSNEPlot.1(B_cells_MCL, pt.size = 1,label = T,do.print = F,
...
g2 <- TSNEPlot.1(B_cells_MCL, pt.size = 1,label = T,do.print = T,
                 label.size = 4, repel = T)
jpeg(paste0(path,"rename_tsne_B~.jpeg"), units="in", width=10, height=7,res=600)
plot_grid(g1+NoLegend(),g2+NoLegend()) +  ggtitle("Rename the clusters")+
        theme(text = element_text(size=15),
              plot.title = element_text(hjust = 0.5))
dev.off()

TSNEPlot.1(B_cells_MCL, pt.size = 1,label = F,do.print = T,no.legend = F,
           label.size = 4, repel = T, title = "6 clusters in B/MCL cells")
save(B_cells_MCL, file = "data/B_cells_MCL_43_20190615.Rda")
...
X6_clusters_markers = read.csv("output/20190609/X6_clusters_FC0.2_markers.csv",row.names = 1)
#B_cells_MCL %<>% ScaleData()
...
B_cells_MCL %<>% ScaleData(features=rownames(B_cells_MCL))
DoHeatmap.1(B_cells_MCL, marker_df = X6_clusters_markers,add.genes = NULL, Top_n = 15, do.print=T, angle = 0,
            group.bar = T, title.size = 20, no.legend = F,size=5,hjust = 0.5,
            label=T, cex.row=5, legend.size = 8,width=10, height=7,
            title = "Top 15 differentially expressed genes in B and MCL clusters")

MakeCorlorBar(B_cells_MCL,  marker_df = X6_clusters_markers,add.genes = markers, Top_n = 15,do.return =F,do.print = T,
...
Idents(B_cells_MCL) <- "X6_orig.ident"

# add color to cluster
B_cells_MCL <- AddMetaColor(object = B_cells_MCL, label= "X6_clusters", 
                            colors = scales::hue_pal()(length(unique(B_cells_MCL$X6_clusters))))
...
for(sample in list_samples$MCL){
        ...
        # SplitTSNEPlot======
        Idents(subset.MCL) = "X6_orig.ident"
        subset.MCL %<>% sortIdent()
        TSNEPlot.1(subset.MCL, split.by = "orig.ident",pt.size = 1,label = F,
                   do.print = T, unique.name = T)
        
        # remove cluster with less than 3 cells======
        table_subset.MCL <- table(subset.MCL$X6_orig.ident) %>% as.data.frame
        keep.MCL <- table_subset.MCL[table_subset.MCL$Freq > 2,"Var1"] %>% as.character()
        
        (X6_cluster <- keep.MCL %>% unique %>% 
                gsub('.*\\_',"",.) %>% as.numeric %>% sort )
        
        print(ident.1 <- paste(samples1,X6_cluster,sep="_"))
        print(ident.2 <- paste(samples2,X6_cluster,sep="_"))
        ...
        GC()
        #DoHeatmap.1======
        subset.MCL %<>% ScaleData(features= rownames(subset.MCL))
        DoHeatmap.1(subset.MCL, marker_df = gde.markers, Top_n = 20, do.print=T, angle = 0,
                    group.bar = T, title.size = 20, no.legend = F,size=5,hjust = 0.5,
                    label=F, cex.row=6, legend.size = NULL,width=10, height=7,unique.name = T,
                    title = paste("Top 20 DE genes in Normal vs.",sample, "B/MCL cells"))
...
for(i in 1:length(list_samples$MCL.1)){#){
        ...
        table_subset.MCL <- table(subset.MCL@meta.data$X6_orig.ident) %>% as.data.frame
        (keep.MCL <- table_subset.MCL[table_subset.MCL$Freq > 2,"Var1"] %>% as.character())
        ...
        DoHeatmap.1(subset.MCL, marker_df = gde.markers, Top_n = 20, do.print=T, angle = 0,
                    group.bar = T, title.size = 20, no.legend = F,size=5,hjust = 0.5,
                    label=F, cex.row=6, legend.size = NULL,width=10, height=7,unique.name = T,
                    title = paste("Top 20 DE genes in",samples1,"vs.",samples2, "B/MCL cells"))
        remove(subset.MCL);GC()
}
...
B_cells_MCL %<>% ScaleData(features = unique(c(VariableFeatures(B_cells_MCL),markers)))
select.markers = unique(c(head(x = VariableFeatures(object = B_cells_MCL), 1000),markers))
y = B_cells_MCL@assays$RNA@scale.data[select.markers,]
y = y[1:1000,1:1000]

system.time(cor.mtr <- fastCor(y,optBLAS = TRUE))
system.time(cor.mtr <- cor(y))

distance <- as.dist(1-cor.mtr)
hc <- hclust(), method="ward.D2")
cc = gsub("_.*","",hc$labels)
