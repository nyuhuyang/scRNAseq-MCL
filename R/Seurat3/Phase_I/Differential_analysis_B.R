...
# load data
B_cells_MCL = readRDS("data/MCL_41_B_20200204.rds")
##############
...
X5clusters_markers <- FindAllMarkers.UMI(B_cells_MCL,logfc.threshold = 0.5,only.pos = T, 
                                          min.pct = 0.1,return.thresh = 0.05)
write.csv(X5clusters_markers,paste0(path,"X5clusters_FC0.2_markers.csv"))

X5clusters_markers = read.csv("Yang/Figure 2/Figure Sources/X5clusters/X5clusters_FC0.01_markers.csv",row.names = 1)
...
if(cycle == T){
        ...
        top = X5clusters_markers[X5clusters_markers$gene %in% cc_genes,] %>% group_by(cluster)
        features = c(as.character(top$gene),
                     tail(VariableFeatures(object = B_cells_MCL), 2),
                     markers)
        B_cells_MCL %<>% ScaleData(features=features)
...
write.csv(X5clusters_normal_markers,paste0(path,"X5clusters_normal_FC0.1_markers.csv"))

X5clusters_normal_markers = read.csv("output/20190904/X5clusters_normal_FC0.1_markers.csv",row.names = 1)
#B_cells_MCL %<>% ScaleData()

markers <- FilterGenes(B_cells_MCL,c("CCND1","CD19","CD5","CDK4","RB1","BTK","SOX11"))
(MT_gene <- grep("^MT-",X5clusters_normal_markers$gene))
X5clusters_markers = X5clusters_normal_markers[-MT_gene,]
Top_n = 40
top = X5clusters_markers %>% group_by(cluster1.vs.cluster2) %>% top_n(Top_n, avg_logFC)
features = c(as.character(top$gene),
...
###############
B_cells_MCL@meta.data$X5clusters_normal_B = as.numeric(B_cells_MCL@meta.data$X5clusters)
B_patients <- grepl("B_cells",B_cells_MCL$manual)
B_cells_MCL@meta.data[B_patients,"X5clusters_normal_B"] = "Patients' B cells"
normal <- B_cells_MCL$orig.ident %in% c("BH","DJ","MD","NZ")
B_cells_MCL@meta.data[normal,"X5clusters_normal_B"] = "Normal"
Idents(B_cells_MCL) = "X5clusters_normal_B"
...
B_cells_MCL@meta.data$X5_orig.ident = paste(B_cells_MCL@meta.data$orig.ident,
                                            B_cells_MCL@meta.data$X5clusters, sep = "_")
...
B_cells_MCL <- AddMetaColor(object = B_cells_MCL, label= "X5clusters", 
                            colors = scales::hue_pal()(length(unique(B_cells_MCL$X5clusters))))
...
for(i in 1:2){
        ...
        DoHeatmap.1(subset.MCL, features = featuresNum,
                    ...
                    group.bar.height = 0,label=T, cex.row=500/length(featuresNum), legend.size = 0,
                    width=10, height=6.5,unique.name = "orig.ident",
...
for(i in 2:length(groups)){
        ...
        clusters = subset.MCL@meta.data[hc$labels,c("orig.ident","X5clusters.colors")]
        #clusters = subset.MCL@meta.data[hc$labels,c("orig.ident")]
        
        cc <- as.character(clusters$X5clusters.colors)
