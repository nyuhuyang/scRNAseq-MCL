...
Remove = c("Pt17_LN1","Pt25_SB1","Pt25_AMB25Pd","Pt28_LN1","Pt11_LN1","PtB13_Ibp","PtB13_Ib1","PtB13_IbR")
samples = samples[!(samples %in% Remove)]
samples %<>% factor(levels = as.character(samples))

Idents(object) = "cell.types"
gene_list <- list("T_cells_CD4+" = c("IL32", "CD52", "TXNIP", "TNFAIP3","EEF1G", "EML4"),
                  "T_cells_CD8+" = c("IL32", "CD52", "LTB", "IFITM1", "JUN", "JUNB", "RPS26",
                             "EEF1G", "EML4", "PIK3R1", "TNFAIP3"))
...
for(i in seq_along(cell.types)){
        ...
        DoHeatmap.matrix(exp[,as.character(S)], features = features,
                         ...
                         group.bar.height = 0,
                         do.print = T,
                         save.path = paste0(path,"Heatmaps/PB_only/scale_average/Doheatmap_",cell.types[i]))
        ...
        if(i == 2){
                DoHeatmap.matrix(exp[,as.character(S)], features = features,
                                 ...
                                 group.bar.height = 0,
                                 do.print = T,
                                 save.path = paste0(path,"Heatmaps/PB_only/scale_average/Doheatmap_",cell.types[i],"_noLegend"))
...
for(i in seq_along(cell.types)){
        ...
        DoHeatmap.matrix(exp[,as.character(S)], features = features,
                         ...
                         group.bar.height = 0,
                         do.print = T,
                         save.path = paste0(path,"Heatmaps/PB_only/average_scale/Doheatmap_",cell.types[i]))
        print(min(exp[,as.character(S)]))
        print(max(exp[,as.character(S)]))
        if(i == 2){
                DoHeatmap.matrix(exp[,as.character(S)], features = features,
                                 ...
                                 group.bar.height = 0,
                                 do.print = T,
                                 save.path = paste0(path,"Heatmaps/PB_only/average_scale/Doheatmap_",cell.types[i],"_noLegend"))
