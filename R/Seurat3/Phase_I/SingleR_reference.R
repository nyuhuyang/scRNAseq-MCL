...
blue_encode_types <- data.frame("original_names" = blueprint_encode$types[-rm_index], # didn't use main_types
                                "sub_types" = colnames(blueprint_encode_rm),
                                stringsAsFactors = F)

blue_encode_types$main_types = plyr::mapvalues(blue_encode_types$original_names,
                                                  from  = Mapvalues$from, 
                                                  to = Mapvalues$to)
blue_encode_types$sub_types %<>% gsub("\\.[[:digit:]]+$","",.) %<>% gsub("\\.$","",.)
...
B_MCL_types <- data.frame("main_types" = c(rep("B-cells",ncol(B_bulk)-1),
                                           rep("MCL-cells",ncol(MCL_bulk)-1)),
                          "sub_types" = colnames(B_MCL_bulk),
                          stringsAsFactors = F)
B_MCL_types$sub_types %<>% paste(B_MCL_types$main_types, .)
...
blueprint_encode_rm[1:3,1:3];B_MCL_bulk[1:3,1:3];GSE107011_data[1:3,1:3]


temp <- merge(blueprint_encode_rm, B_MCL_bulk,
                         by="row.names",all=FALSE)
temp %<>% RemoveDup()
blue_encode_MCL_GSE107011 <- merge(temp, log1p(GSE107011_data),
                                   by="row.names",all=FALSE)
blue_encode_MCL_GSE107011 %<>% RemoveDup()

testMMM(blue_encode_MCL_GSE107011)

jpeg(paste0(path,"boxplot_blue_encode_MCL_GSE107011.jpeg"), units="in", width=10, height=7,res=600)
par(mfrow=c(1,1))
boxplot(blue_encode_MCL_GSE107011) #slow
title(main = "boxplot for Blueprint + Encode + MCL+ GSE107011")
dev.off()

# Create Singler Reference
ref = CreateSinglerReference(name = 'MCL_blue_encode_GSE107011',
                             expr = as.matrix(blue_encode_MCL_GSE107011), # the expression matrix
                             types = c(blue_encode_types$sub_types,
                                       B_MCL_types$sub_types ,
                                       GSE107011_types$sub_types), 
                             main_types = c(blue_encode_types$main_types,
                                            B_MCL_types$main_types,
                                            GSE107011_types$main_types))

saveRDS(ref,file='data/ref_MCL_blue_encode_GSE107011_20200605.rds')
