...
singler = readRDS(file="output/singlerF_MCL_41_20200605.rds")
...
singlerDF = data.frame("singler1sub" = singler$singler[[1]]$SingleR.single$labels,
                       ...
                       stringsAsFactors = F)
singlerDF$sub_types %<>% gsub("T-cells CD", "T-cells.CD",.)
singlerDF$main_types = gsub("\\ .*","", singlerDF$sub_types)
singlerDF = singlerDF[colnames(object),]
singlerDF$orig.ident = object$orig.ident
##############################
...
table(singlerDF$main_types, singlerDF$orig.ident) %>% kable %>%
        kable_styling()
##############################
# adjust cell label
##############################
# combine main cell types
Mapvalues = data.frame(c("Erythrocytes|Macrophages|Megakaryocytes|PBMCs",   "Myeloid cells"),
                       c("MEP|CLP|HSC|CMP|GMP|MPP|Plasmablasts|Progenitor",       "HSC/progenitors"),
                       c("Adipocytes|Endothelial-cells",       "Nonhematopoietic cells"),
