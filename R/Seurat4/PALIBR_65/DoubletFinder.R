...
for (i in 1:length(object_list)) {
    sweep.res_list[[i]] <- paramSweep_v3(object_list[[i]], PCs = 1:npcs, sct = F)
    Progress(i,length(object_list))
}
saveRDS(sweep.res_list,file = "output/MCL_65_20220411_sweep.res_list.rds")
sweep.res_list = readRDS(file = "output/MCL_65_20220411_sweep.res_list.rds")
...
for(i in 1:length(object_list)){
    ...
    nExp_poi <- round(Multiplet_Rate(object_list[[i]])*nrow(object_list[[i]]@meta.data))  ## Assuming 7.5% doublet formation rate - tailor for your dataset
    nExp_poi.adj <- round(nExp_poi*(1-homotypic.prop))

    ## Run DoubletFinder with varying classification stringencies ----------------------------------------------------------------
    object_list[[i]] <- doubletFinder_v3(seu = object_list[[i]], PCs = 1:npcs,
